...
export const meta: MetaFunction = () => {
  return [
    { title: "Delta Force - Loadouts" },
    {
      name: "description",
      content:
        "Explore and share you build codes for Warfare and Operation modes!",
    },
  ];
};
...
async function getBuilds(
  ...
  serverFilter?: string,
  modeFilter?: string,
  seasonFilter?: SeasonQuickFilter
): Promise<Build[]> {
  const searchFilter = searchQuery
    ? `(title ~ "${searchQuery}"  || description ~ "${searchQuery}"  || weapon ~ "${searchQuery}" || author ~ "${searchQuery}" || code ~ "${searchQuery}")`
    : null;
  ...
  return (
    <div className="min-h-screen m-1 lg:m-8">
      <header className="bg-background p-4 flex justify-center items-center mt-20">
        ...
        />
        <div>
          <h1 className="text-xl font-bold flex flex-shrink items-center justify-center">
            Loadouts for{" "}
            <span className="pl-1 text-green-400">Delta Force</span>
          </h1>
          <div>
      ...
      <main className="p-4 pt-8 lg:p-6 lg:pt-12">
        <div className="space-y-10 mb-12">
          <SearchInput
            ...
            createBuildModalOpen={createBuildModalOpen}
          />
